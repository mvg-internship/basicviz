...
int main() {
    ...
    transformation_graph_with_printing( nodes );
...
void greedyFAS( std::vector<tree_node> &nodes, std::vector< std::pair<tree_node, tree_node> > &deleted_edges ) {
        ...
        for( int j = 0; j < s[i].succ.size(); j++ ) {
            for( int k = 0; k < i; k++ ) {
                if ( s[i].succ[j] == s[k].id ) {
                    tree_node succ_node = get_tree_node_by_id( copy_nodes, s[i].succ[j] );
                    std::pair<tree_node, tree_node> edge = {s[i], succ_node};
...
void add_dummy_nodes( std::vector<tree_node> &nodes, std::vector< std::pair<tree_node, tree_node> > deleted_edges ) {
    int flag = 1;
    while( flag ) {
        flag = 0;
        for( int i = 0; i < nodes.size(); i++ ) {
    ...
    for( int i = 0; i < deleted_edges.size(); i++ ) {
        if( deleted_edges[i].first.layer == deleted_edges[i].second.layer + 1) {
            for( int j = 0; j < nodes.size(); j++ ) {
                if( nodes[j].id == deleted_edges[i].first.id ) {
                    nodes[j].succ.push_back(deleted_edges[i].second.id);
                }
                if( nodes[j].id == deleted_edges[i].second.id ) {
                    nodes[j].pred.push_back(deleted_edges[i].first.id);
                }
            }
        }
        else {
            tree_node first_node = get_tree_node_by_id( nodes, deleted_edges[i].first.id );
            tree_node second_node = get_tree_node_by_id( nodes, deleted_edges[i].second.id );
...
void transformation_graph_with_printing( std::vector<tree_node> &nodes ) {
    pred_reinit( nodes );
    deg_reinit( nodes );
    
    printf("Before: \n");
    topology_heapSort( nodes );
    DFS_cycle_exists( nodes );
    print_info( nodes );
        

    std::vector< std::pair<tree_node, tree_node> > deleted_edges = {};
    greedyFAS( nodes, deleted_edges );
    pred_reinit( nodes );
    deg_reinit( nodes );
                
    printf("After greedyFAS: \n");
    DFS_cycle_exists( nodes );
    print_info( nodes );
    ...
    for( int i = 0; i < deleted_edges.size(); i++ ) {
        printf( "%i, %i\n", deleted_edges[i].first.id, deleted_edges[i].second.id);
    }
    ...
    printf("After addition dummy nodes: \n");
    print_info( nodes );
}
